...
def collate_fn(batch):
    ...
    prompt_embeds = repeat(prompt_embeds, 'b ... -> (b 2) ...')  # Assuming 2 frames per video
    pooled_prompt_embeds = torch.stack([item['pooled_prompt_embeds'] for item in batch]) 
    pooled_prompt_embeds = repeat(pooled_prompt_embeds, 'b ... -> (b 2) ...')  # Assuming 2 frames per video 
...
class VideoPairedCaptionDataset(data.Dataset):
    ...
    def get_frame_indices(self):
        ...
        for video_path in self.rel_path_list:
            video_length = 30
            ref_image_idxs.append(random.randint(1, video_length - 1)) # Start from 1 to avoid the first frame
    ...
    def __getitem__(self, index):
        ...
        lq_latent = torch.cat([lq_prev_latent, lq_curr_latent], dim=0)  # (2, C, H, W)
        gt_latent = torch.cat([gt_prev_latent, gt_curr_latent], dim=0)  # (2, C, H, W)
...
if __name__ == "__main__":
    ...
    dataset = VideoPairedCaptionDataset(
        root_folder='/mnt/dataset2/jaewon/YouHQ/YouHQ-Train',
        null_text_ratio=0.0,
        use_sea_raft=True,
    )
    ...
    for batch in tqdm(train_loader, desc="Training Loader"):
        ...
        print(f'Arrange LR latent shape: {rearrange(batch["conditioning_pixel_values"], "(b f) c h w -> b f c h w", f=2)[:,0,:,:,:].shape}')
