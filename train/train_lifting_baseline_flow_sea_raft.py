...
def _sea_raft_forward_flow(args, model, image1, image2):
...
def _sea_raft_calc_flow(args, model, image1, image2):
    img1 = F.interpolate(image1, scale_factor=2 ** args.scale, mode="bilinear", align_corners=False)
    img2 = F.interpolate(image2, scale_factor=2 ** args.scale, mode="bilinear", align_corners=False)
    flow, info = _sea_raft_forward_flow(args, model, img1, img2)
    flow_down = (
        F.interpolate(flow, scale_factor=0.5 ** args.scale, mode="bilinear", align_corners=False)
        * (0.5 ** args.scale)
    )
...
def _load_sea_raft_model(cfg_path, model_path):
    sea_raft_root = os.path.join(os.getcwd(), "SEA_RAFT")
    sea_raft_core = os.path.join(sea_raft_root, "core")
    if not os.path.isdir(sea_raft_core):
        raise FileNotFoundError(f"SEA_RAFT core directory not found: {sea_raft_core}")

    args = None
    saved_modules = {}
    for name in _SEA_RAFT_MODULES:
        if name in sys.modules:
            saved_modules[name] = sys.modules.pop(name)

    sys.path.insert(0, sea_raft_root)
    ...
    try:
        ...
        raft_path = os.path.join(sea_raft_core, "raft.py")
        spec = importlib.util.spec_from_file_location("sea_raft_core.raft", raft_path)
        sea_raft_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(sea_raft_module)
        model = sea_raft_module.RAFT(args)
        load_ckpt(model, model_path)
    finally:
        sys.path.remove(sea_raft_core)
        sys.path.remove(sea_raft_root)
...
class SeaRaftFlowExtractor(torch.nn.Module):
    def __init__(self, cfg_path, model_path):
        super().__init__()
        self.model, self.args = _load_sea_raft_model(cfg_path, model_path)
        self.model.eval()

    def forward(self, image1, image2, iters=12, test_mode=True):
        image1 = image1.float() * 255.0
        image2 = image2.float() * 255.0
        prev_iters = self.args.iters
        if iters is not None:
            self.args.iters = iters
        flow, _ = _sea_raft_calc_flow(self.args, self.model, image1, image2)
        self.args.iters = prev_iters
        return None, flow
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--sea_raft_cfg",
        type=str,
        default="./SEA_RAFT/config/eval/spring-M.json",
        help="Path to SEA-RAFT config JSON.",
    )
    parser.add_argument(
        "--sea_raft_model_path",
        type=str,
        default="./SEA_RAFT/models/Tartan-C-T-TSKH-spring540x960-M.pth",
        help="Path to SEA-RAFT weights.",
    )
...
def main(args):
    ...
    if args.stage1_raft_weight is not None:
        ...
        raft = raft_tools.get_raft_model(args.raft_model_path,
                                     ...
                                     use_context_dpt=args.use_context_dpt,
                                     use_custom_dpt=args.use_custom_dpt,)
    flow_extractor = SeaRaftFlowExtractor(
        cfg_path=args.sea_raft_cfg,
        model_path=args.sea_raft_model_path,
    )
    logger.info(f"Loaded SEA-RAFT flow extractor from {args.sea_raft_model_path} with {args.sea_raft_cfg}.")
