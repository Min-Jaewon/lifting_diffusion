...
def parse_args(input_args=None):
    ...
    parser.add_argument("--feature_upsample", type=str, choices=['bilinear', 'conv'], default='bilinear', help="Feature upsample method in RAFT.")
    ...
    parser.add_argument("--wandb_project", type=str, default='DiT4SR_OVERFITTING', help="Weights and Biases project name.")
...
def viz(gt_img,lq_img, flow_pred, flow_gt, flow_lq=None):
    ...
    results = []
    gt_img = rearrange(gt_img, '(b f) c h w -> b f c h w', f=2)
...
def validation_step(args, accelerator, transformer, raft, val_dataloader, weight_dtype,
                    noise_scheduler_copy, get_sigmas, target_features):
    ...
    for b_idx, batch in enumerate(tqdm(val_dataloader, desc="Validation")):
        with torch.no_grad():
            ...
            diffusion_features = target_features[0] # Only one target feature for now
            lr_image = batch["lq_image"].to(dtype=model_input.dtype)
            lr_image1 = rearrange(lr_image, '(b f) c h w -> b f c h w', f=2)[:,0,:,:,:] # take the first frame
            flow_preds = raft(lr_image1, diffusion_features, iters=12,)
            ...
            diff_total_loss += (loss.item() - flow_loss.item())
            ...
            caption = f'{video_id}_frame_{frame_idx:02d}'
...
def main(args):
    ...
    dataset = VideoPairedCaptionDataset(root_folder=args.root_folders, 
                                         null_text_ratio=args.null_text_ratio,)
    # Overfit to a smaller dataset for debugging purposes
    train_dataset = torch.utils.data.Subset(dataset, list([2000]))
    # val_dataset = torch.utils.data.Subset(dataset, [10000, 10010, 10020, 10030, 10040, 10050, 10060, 10070, 10080, 10090])
    ...
    transformer, raft, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        transformer, raft, optimizer, train_dataloader, lr_scheduler
    )
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer):
                ...
                if accelerator.sync_gradients:
                    ...
                    params_to_clip = raft_parameters + transformer_parameters
            ...
            if accelerator.sync_gradients:
                    ...
                    accelerator.log(
                        {
                            'train/total_loss': loss.detach().item(),
                            'train/flow_loss': flow_loss.detach().item(),
                            'train/diff_loss': (loss - flow_loss).detach().item(),
                            'train/lr': lr_scheduler.get_last_lr()[0],
                        }
                    )
                if accelerator.is_main_process:
                ...
                if global_step % args.validation_steps == 0 or global_step == 1:
                    ...
                    for idx, pil_image in enumerate(pil_images):
                        ...
                        caption = f'{video_id}_frame{frame_idx:02d}'
                        accelerator.log(
                            {f'train/flow_viz_{idx}': wandb.Image(pil_image, caption=caption)}, step=global_step
                        )
                    
                #     # Validation step
                #     logs = validation_step(args,  accelerator, transformer, raft, val_dataloader, weight_dtype, 
                #                         noise_scheduler_copy, get_sigmas, target_features,)
