    ...
    for b_idx, batch in enumerate(tqdm(val_dataloader, desc="Validation")):
        with torch.no_grad():
            ...
            if vae is not None:
                ...
                model_pred_img = rearrange(model_pred_img, '(b f) c h w -> b f c h w', f=2)
                gt_img = rearrange(batch['gt_image'], '(b f) c h w -> b f c h w', f=2)
                lq_img = rearrange(batch['lq_image'], '(b f) c h w -> b f c h w', f=2)
                
                timesteps = rearrange(timesteps, '(b f) -> b f', f=2)[:,0]
...
def main(args):
    ...
    transformer, raft, optimizer, train_dataloader, val_dataloader, lr_scheduler = accelerator.prepare(
        transformer, raft, optimizer, train_dataloader, val_dataloader, lr_scheduler
    )
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.validation_steps == 0 or global_step == 1:
                    ...
                    if not args.only_train_raft:
                        ...
                        model_pred_image = rearrange(model_pred_image, '(b f) c h w -> b f c h w', f=2)
                        
                        gt_image = rearrange(batch['gt_image'], '(b f) c h w -> b f c h w', f=2)
                        lq_image = rearrange(batch['lq_image'], '(b f) c h w -> b f c h w', f=2)
