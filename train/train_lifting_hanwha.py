...
from dataloaders.paired_dataset_sd3_latent_hanwha import VideoPairedCaptionDataset, collate_fn
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--lifting_frames", type=int, default=3, help="Number of input frames for lifting.")
    # Parse the arguments
...
def validation_loop(args, accelerator, transformer, raft, val_dataloader, weight_dtype,
    ...
    for b_idx, batch in enumerate(tqdm(val_dataloader, desc="Validation")):
        with torch.no_grad():
            model_input = batch["gt_image"].to(device, dtype=weight_dtype)
            controlnet_image = batch["lq_image"].to(device, dtype=weight_dtype)
            ...
            u = repeat(u, 'b -> (b 3)') # since each data point has two framesb
...
def main(args):
    ...
    def get_input_hook(name):
        def hook(module, input, output):
            if output[0].shape[-1] == 64: # key feature of attention
                temp = []
                for f in output:
                    temp.append(rearrange(f, 'h (bf t) d -> bf t (h d)', t=1024))
                feature = torch.concat(temp, dim=0)
            else:
    ...
    flow_extractor.requires_grad_(False) # Do not train flow extractor
            ...
            if accelerator.is_main_process:
                ...
                while len(weights) > 0:
                    ...
                    try:
                        ...
                        model_name = 'raft'
                        raft_model = unwrap_model(model)
                        torch.save(model.state_dict(), os.path.join(output_dir, f"{raft_model}_weights.pt"))
    ...
    image_logs = None
    num_frames = args.lifting_frames
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer):
                # Convert images to latent space
                model_input = batch["gt_image"].to(dtype=weight_dtype)
                with torch.no_grad():
                    model_input = vae.encode(model_input).latent_dist.sample()
                    model_input = (model_input - vae.config.shift_factor) * vae.config.scaling_factor
                # controlnet(s) inference
                controlnet_image = batch["lq_image"].to(dtype=weight_dtype)
                with torch.no_grad():
                ...
                u = repeat(u, 'b -> (b f)', f=num_frames) # repeat for num_frames
                indices = (u * noise_scheduler_copy.config.num_train_timesteps).long()
                timesteps = noise_scheduler_copy.timesteps[indices].to(device=model_input.device)
                # Add noise according to flow matching.
                # zt = (1 - texp) * x + texp * z1
                ...
                pooled_prompt_embeds = prompts["pooled_prompt_embeds"]
                prompt_embeds = repeat(prompt_embeds, 'b ... -> (b f) ...', f=num_frames)
                pooled_prompt_embeds = repeat(pooled_prompt_embeds, 'b ... -> (b f) ...', f=num_frames)
                ...
                diffusion_features = target_features # list of features (frame, T, C)
                # (B*F, T, C) -> (B, F, T, C)
                df_bf = [
                    rearrange(feat, '(b f) t c -> b f t c', f=num_frames)
                    for feat in diffusion_features
                ]
                # (B, F-1, 2, T, C)
                df_bf = [
                    torch.stack([df[:, :-1], df[:, 1:]], dim=2)
                    for df in df_bf
                ]
                # (2*B*(F-1), T, C)
                reshaped_df = [
                    rearrange(df, 'b f p t c -> (b f p) t c').contiguous()
                    for df in df_bf
                ]
                ...
                # (b, f, c, h, w)
                lr_bf = rearrange(lr_image, '(b f) c h w -> b f c h w', f=num_frames)
                gt_bf = rearrange(gt_image, '(b f) c h w -> b f c h w', f=num_frames)
                ...
                # (b*(f-1), c, h, w)
                lr_image1 = rearrange(lr_image1, 'b f c h w -> (b f) c h w').contiguous().clone()
                lr_image2 = rearrange(lr_image2, 'b f c h w -> (b f) c h w').contiguous().clone()

                gt_image1 = rearrange(gt_image1, 'b f c h w -> (b f) c h w').contiguous()
                gt_image2 = rearrange(gt_image2, 'b f c h w -> (b f) c h w').contiguous()
                
                _, flow_gt = flow_extractor(gt_image1, gt_image2, iters=20, test_mode=True)
                _, flow_lq = flow_extractor(lr_image1, lr_image2, iters=20, test_mode=True)
                flow_preds = raft(lr_image1, lr_image2, reshaped_df, iters=12,)
