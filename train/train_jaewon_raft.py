...
@torch.no_grad()
def validation_loop(args, accelerator, raft, val_dataloader, weight_dtype, vae=None):
    ...
    for b_idx, batch in enumerate(tqdm(val_dataloader, desc="Validation")):
        with torch.no_grad():
            bsz = batch['flow_gt'].shape[0] 
            
            lr_image = batch["lq_image"].to(dtype=weight_dtype)
            lr_image1 = rearrange(lr_image, '(b f) c h w -> b f c h w', f=2)[:,0,:,:,:] # take the first frame
            lr_image2 = rearrange(lr_image, '(b f) c h w -> b f c h w', f=2)[:,1,:,:,:] # take the second frame
            
            flow_preds = raft(lr_image1, lr_image2, iters=12,)
            flow_gt = batch['gt_flow'].to(device, dtype=weight_dtype)
...
def main(args):
    ...
    num_trainable_params = sum(p.numel() for p in raft_parameters)
    ...
    params_to_optimize = raft_parameters
    ...
    train_dataset = torch.utils.data.Subset(dataset, list(range(args.dataset_size)))
    ...
    raft, optimizer, train_dataloader, val_dataloader, lr_scheduler = accelerator.prepare(
        raft, optimizer, train_dataloader, val_dataloader, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(raft):
                ...
                lr_image2 = rearrange(lr_image, '(b f) c h w -> b f c h w', f=2)[:,1,:,:,:] # take the second frame
                flow_preds = raft(lr_image1, lr_image2, iters=12,)
                flow_gt = batch['gt_flow'].to(dtype=weight_dtype)
                flow_loss, metrics = raft_tools.sequence_loss(
                    flow_preds,
                    flow_gt,
                    torch.ones_like(flow_gt[:, :1, :, :]),
                ...
                if accelerator.sync_gradients:
                    ...
                    params_to_clip = raft_parameters
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.validation_steps == 0 or global_step == 1:
                    ...
                    logs = validation_loop(args, accelerator, raft, val_dataloader, weight_dtype, None)
