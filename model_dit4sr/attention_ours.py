...
class JointTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_attention_heads, attention_head_dim, context_pre_only=False, qk_norm: Optional[str] = None, use_dual_attention: bool = False, is_full_attention=False,
                 full_attn_mode='sequential'):
        super().__init__()
        ...
        if is_full_attention:
            self.full_attn_mode = full_attn_mode
        ...
        if use_dual_attention or is_full_attention:
    ...
    def forward(
        ...
        if self.use_dual_attention:
            ...
            # 1. 시퀀스 정보 설정 (학습 시의 num_frames와 일치해야 함)
            num_frames = 3 
            
            # 2. Sequential 처리를 위한 두 번째 정규화 (F.layer_norm 활용)
            # 기존 gate_msa2가 없으므로, 데이터 분포를 안정화시킨 후 attn2에 입력합니다.
            if self.full_attn_mode == 'sequential':
                norm_hidden_states2 = F.layer_norm(hidden_states, (hidden_states.shape[-1],))
            elif self.full_attn_mode == 'parallel':
                norm_hidden_states2 = norm_hidden_states
            
            # 3. Temporal Attention을 위한 차원 변형 (Lifting)
            # (Batch*Frames, 2*N, Dim) -> 이미지와 컨트롤 분리
            h_img, h_ctrl = norm_hidden_states2.chunk(2, dim=1) 
            
            # (Batch*Frames, N, Dim) -> (Batch, Frames*N, Dim) : 시간축으로 이어붙임
            h_img = rearrange(h_img, '(b f) t c -> b (f t) c', f=num_frames).contiguous()
            h_ctrl = rearrange(h_ctrl, '(b f) t c -> b (f t) c', f=num_frames).contiguous()
            
            # 다시 이미지+컨트롤 결합하여 attn2 입력 생성
            temporal_input = torch.cat([h_img, h_ctrl], dim=1)
            
            # 4. Full Attention (Temporal) 연산
            attn_output2 = self.attn2(hidden_states=temporal_input)
            
            # 5. 원래 차원으로 복구 (Inverse Rearrange)
