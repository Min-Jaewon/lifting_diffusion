...
from pipelines.pipeline_dit4sr_video_jaeeun_viz import StableDiffusion3ControlNetPipeline
...
def load_dit4sr_pipeline(args, accelerator):
    ...
    validation_pipeline = StableDiffusion3ControlNetPipeline(
        vae=vae, text_encoder=text_encoder_one, text_encoder_2=text_encoder_two, text_encoder_3=text_encoder_three, 
        tokenizer=tokenizer_one, tokenizer_2=tokenizer_two, tokenizer_3=tokenizer_three, 
        transformer=transformer, scheduler=scheduler, raft=raft, debug_viz=args.debug_viz, debug_viz_steps=args.debug_viz_steps
    )
    ...
    if accelerator.is_main_process:
        ...
        for image_idx, (video_file, prompt_name) in enumerate(zip(video_list, prompt_list)):
            ...
            if (org_width // 8) * (org_height // 8) <= args.latent_tiled_size **2:
                ...
                if org_width * org_height != int(np.sqrt(org_width * org_height)) **2:
                    try:
                        pass
                    except Exception as e:
                        passed_video_list.append(video_file)
                        print(f'Skipped {video_file} due to non-square number size: {org_width}x{org_height}')
                        continue
                ...
                padding_flag = True
                base = 16
                # Size must be multiple of 'base'
                new_width = ((org_width - 1) // base + 1) * base
                new_height = ((org_height - 1) // base + 1) * base
                print(f'Padding {org_width}x{org_height} to {new_width}x{new_height}')
                padded_frames = []
                for frame in frames:
                    padded_frame = Image.new('RGB', (new_width, new_height))
            ...
            with torch.autocast("cuda"):
                ...
                try:
                    ...
                    print(f'Error during inference of video {video_basename}: {e}')
                    continue
                end_time = time.time()
            ...
            for i_frame in range(len(images)):
                ...
                if padding_flag:
                    ...
                    frame = frame[:, :org_height, :org_width]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--debug_viz_steps", type=str, default="")
